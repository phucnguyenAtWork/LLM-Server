...
# Chapter 5 — AI Model Results, Generalization, Stability, Baseline Comparison, Failure Analysis

> **Chapter discipline.** Chapter 4 covers implementation only. All evaluation
> results, generalization analysis, stability claims, baseline comparisons, and
> failure analysis live here. Sections below embed tables generated by
> `run_thesis_matrix.py` and `thesis_evaluation.py` so the chapter stays in sync
> with the latest run.

## 5.1 Threshold pass-rates and failure-mode breakdown

Each per-test row is reduced to a binary PASS / FAIL using fixed thresholds:

| Metric                  | Threshold       |
|-------------------------|-----------------|
| financial_accuracy      | ≥ 0.80          |
| role_appropriateness    | ≥ 0.70          |
| personalization_quality | ≥ 0.60          |
| hallucination_rate      | ≤ 0.10          |
| JSON schema parse       | parses to `ModelOutput` |

Because expected labels are always PASS in this benchmark, a 2×2 matrix would
be degenerate. Reporting is therefore **threshold pass-rate per metric** and a
**failure-mode breakdown** (which threshold tripped first, by role) — not a
classification confusion matrix. Generated tables: see
`logs/thesis_eval_*.json` → `pass_rates.overall` and `pass_rates.failure_modes`.

## 5.2 Role classification confusion matrix and macro-F1

The only legitimate classification artifact in this thesis is the 3×3 over the
three personas. For each response, `infer_target_role()` in `thesis_evaluation.py`
scores the message against each role's vocabulary and picks argmax; that
prediction is compared against the case's true `role`. Reported per-cell:
3×3 confusion matrix, per-role precision / recall / F1, and macro-F1.
Generated table: `logs/thesis_eval_*.json` → `role_classification`.

## 5.3 Generalization slice — 18 diverse cases

The generalization claim is grounded on a curated 18-case slice (6 per role
× 3 difficulty bands: easy / multi-step / hard) defined in
`benchmark_cases_diverse.py`. The full 141 cases stay as the comprehensive run.
Quote both: the diverse slice for stability and generalization headlines, the
full run for thoroughness. Reproduce with:

```
python benchmark.py --adapter financial_qwen_native_v8 --cases diverse --rag-backend none
```

## 5.4 Baseline-vs-fine-tuned grid

Four-cell headline matrix that isolates one variable at a time:

| Cell | Model                       | RAG    | Isolates                              |
|------|-----------------------------|--------|---------------------------------------|
| B1   | Untuned Qwen2.5-3B-Instruct | none   | raw base-model capability             |
| B2   | Untuned Qwen2.5-3B-Instruct | vector | RAG gain on the base model            |
| M1   | v8 (QLoRA)                  | none   | gain from QLoRA fine-tuning alone     |
| M2   | v8 (QLoRA)                  | vector | combined fine-tuning + RAG (deployed) |

Quoted lifts:
- B1 → M1 = QLoRA fine-tuning lift
- B1 → B2 = RAG-on-base lift
- B2 → M2 = fine-tuning lift on top of RAG
- M1 → M2 = RAG lift on the deployed model

Full grid extends to {baseline, v6, v7, v8} × {none, oracle, vector}.
Reproduce with:

```
python run_thesis_matrix.py --cells headline --cases diverse --mode deterministic
python run_thesis_matrix.py --cells full     --cases full    --mode deterministic
```

## 5.5 Stability — two distinct experiments

**5.5a Deterministic reproducibility (deployment behavior).**
`do_sample=False`, fixed seed, identical inputs. Each cell is run twice and
benchmark responses are compared byte-for-byte. The thesis quotes a single
boolean per cell ("reproducible: yes/no"). This is the configuration the
deployed FINA pipeline uses.

```
python run_thesis_matrix.py --mode reproducibility --cells headline --cases diverse
```

**5.5b Sampling robustness (sensitivity study, NOT deployment).**
`do_sample=True, temperature=0.3`, N=3 seeds. The thesis quotes mean ± std per
metric per cell. This is framed explicitly as a sensitivity study; deployment
remains the deterministic configuration in §5.5a.

```
python run_thesis_matrix.py --mode sampling --seeds 0 1 2 --cells headline --cases diverse
```

## 5.6 Retrieval-quality metrics

When the vector backend is used and oracle source IDs are available (carried
in `oracle_ids` by `benchmark.py`), `compute_retrieval_quality()` in
`thesis_evaluation.py` reports **Precision@k**, **Recall@k**, and **MRR**
across the run. Generated table: `logs/thesis_eval_*.json` → `retrieval_quality`.

## 5.7 Failure analysis

Each evaluation run exports three thesis-appendix CSVs:

- `<stem>_easy_success_cases.csv` — all metrics PASS and `financial_accuracy == 1.0`.
- `<stem>_hard_success_cases.csv` — cases in the diverse-slice "hard" band that the model passed.
- `<stem>_failure_cases.csv` — at least one metric FAIL.

Columns: `id, role, difficulty, user_input, expected_output, model_output,
financial_accuracy, role_appropriateness, personalization_quality,
hallucination_rate, citation_correctness, json_ok, failure_type, explanation`.
`failure_type` is the first threshold tripped (or `schema_invalid`);
`explanation` is auto-generated from the per-metric scoring helpers.

## 5.8 Train / val / test split provenance

`train.py` writes a deterministic 80/10/10 split (seed 42) to `splits/`:
`train_ids.txt`, `val_ids.txt`, `test_ids.txt`, and `manifest.json` (split
sizes, role distribution, family distribution, overlap check). A boot-time
assertion verifies that no benchmark case prompt hashes into the training set,
guaranteeing no test leakage. Reproduce the split without training:
